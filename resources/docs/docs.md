...
The `home-routes` handler in the `luhn-server.routes.home` namespace
...
The `home-page` function will in turn call the `luhn-server.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/luhn_server/core.cljs` file.



<p class="title is-5">Organizing the routes</p>

The routes are aggregated and wrapped with middleware in the `luhn-server.handler` namespace:
...
Request middleware functions are located under the `luhn-server.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `luhn-server.dev-middleware` namespace found in
